// ==UserScript==
// @name         Blutopia BON Giveaway (No Rig)
// @namespace    https://openuserjs.org/users/Nums
// @description  Enables the functionality to become poor, with no rigging flare features
// @version      6.1.3-mns.1
...
(function() {
    ...
    const HOST_PANEL_SECTION_ORDER = Object.freeze(["giveaway", "stats", "pot", "entry", "help", "fun"]);
    ...
    const HOST_PANEL_COMMAND_ORDER_BY_SECTION = Object.freeze({
        giveaway: Object.freeze(["reminder", "entries", "time", "addtime", "removetime", "winners"]),
    ...
    const baseCommands = ["time", "entries", "help", "commands", "bon", "range", "gift","random", "number", "free", "lucky", "luckye", "stats", "top", "most", "sponsors", "unlucky", "largest",];
...
>
  ...
  >
    Start
      ...
      <!-- Presets -->
      <div class="giveaway-presets-row" style="display:flex; align-items:center; justify-content:center; gap:6px; flex-wrap:wrap; margin-top:4px;">
  ...
  `;
    ...
    class SponsorTracker {
        ...
        announceWinnerScalingIfNeeded() {
            ...
            const threshold = Math.max(1, Math.floor(initialPotVerified / baseWinners));
    ...
    Object.assign(COMMAND_HANDLERS, {
        ...
        free({ safeAuthor, giveawayData , reply}) {
            ...
            reply(`Free numbers: ${sample.join(", ")}.`);
        },
        ...
        winners(ctx) {
            ...
            if (giveawayData.scaleWinnersWithSponsors) {
                giveawayData.hostMaxScaledWinners = Math.max(
                    newCount,
                    Math.min(Number(giveawayData.hostMaxScaledWinners) || newCount, MAX_WINNERS)
                );
                if (maxScaledWinnersInput) {
                    maxScaledWinnersInput.min = String(newCount);
                    if (Number(maxScaledWinnersInput.value) < newCount) {
            ...
            initializeScaledWinnersAnnouncementState(giveawayData);
            const effective = Math.floor(Number(giveawayData.effectiveWinnersNum) || newCount);
            const scalingInfo = (giveawayData.scaleWinnersWithSponsors && effective !== newCount)
                ? ` (currently [b][color=#5DE2E7]${effective}[/color][/b] with scaling)`
                : "";
            reply(`Number of winners set to [color=#1DDC5D][b]${newCount}[/b][/color].${scalingInfo}`);
            snapshotGiveaway();
        },
    ...
    function formatFreeNumberSuggestion(giveawayData) {
        ...
        return ` Here are some free numbers you can try: [b][color=#1DDC5D]${sample.join(", ")}[/color][/b].`;
    }
    ...
    function recomputeEffectiveWinners(data) {
        ...
        if (data.scaleWinnersWithSponsors) {
            const initialPotVerified = Math.max(0, Math.floor(Number(data.initialPotVerifiedAtStart) || 0));
            const baseBonPerWinner = Math.max(1, Math.floor(initialPotVerified / baseWinners));
            const totalContribForScaling = getTotalContribForScaling(data);
    ...
    function getSponsorshipNextWinnerLine(data, options = {}) {
        ...
        const initialPotVerified = Math.max(0, Math.floor(Number(data.initialPotVerifiedAtStart) || 0));
        const thresholdBonPerWinner = Math.max(1, Math.floor(initialPotVerified / baseWinners));
        ...
        const remaining = (thresholdBonPerWinner - progress) % thresholdBonPerWinner;
